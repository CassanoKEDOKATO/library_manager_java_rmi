...
// the server to uniquely identify the registered client.
    ...
    public Response createBookCopy(BookCopy bookCopy, boolean isCallFromSever) throws RemoteException {
        try {
            String query = "INSERT INTO book_copy (year_published, published_id, book_id ) VALUES (?, ?, ?)";
            pst = conn.prepareStatement(query);
    ...
    public Response deleteBookCopy(int id, boolean isCallFromSever) throws RemoteException {
        try {
            ...
            int rowsAffected = deleteBookCopyStatement.executeUpdate();
        ...
        try  {
            ...
            if (rowsUpdated > 0) {
                if (checkout.isIs_returned()) {
                    ...
                    notification.setMessage("Notification: Thủ thư đã duyệt yêu cầu bạn mượn sách!💕");
    ...
    public Response registerClient(Patron patron) throws RemoteException {
        try {
            ...
            String insertNotificationQuery = "INSERT INTO patron_account (first_name, last_name, email, password,status) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement insertNotificationStatement = conn.prepareStatement(insertNotificationQuery, Statement.RETURN_GENERATED_KEYS);
            insertNotificationStatement.setString(1, patron.getFirstName());
            insertNotificationStatement.setString(2, patron.getLastName());
            insertNotificationStatement.setString(3, patron.getEmail());
            insertNotificationStatement.setString(4, patron.getPassword());
            insertNotificationStatement.setInt(5, 1);
            insertNotificationStatement.executeUpdate();
